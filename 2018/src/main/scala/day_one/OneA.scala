...
/*
More info in https://adventofcode.com/2018/day/1
...
applied?
 */
case class OneA(frequencies: List[Int]) {
...
object OneA {
  val frequencies = Source.fromFile("inputs/input_one.txt").getLines.toList.map(_.toInt)

  def start() = OneA(frequencies)
}
